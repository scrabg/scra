...
class UserService:
    ...
    async def reset_user_services(cls, query_db: AsyncSession, page_object: ResetUserModel):
        ...
        if page_object.password:
            try:
                if len(page_object.password.encode('utf-8')) > 72:
                    raise ServiceException(message='新密码长度不能超过72字节')
            except Exception:
                raise ServiceException(message='密码编码失败，请使用有效字符')
        if page_object.old_password:
            try:
                if len(page_object.old_password.encode('utf-8')) > 72:
                    raise ServiceException(message='旧密码长度不能超过72字节')
            except Exception:
                raise ServiceException(message='密码编码失败，请使用有效字符')
