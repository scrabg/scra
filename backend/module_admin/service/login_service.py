...
class LoginService:
    ...
    async def authenticate_user(cls, request: Request, query_db: AsyncSession, login_user: UserLogin):
        ...
        # 预校验：bcrypt 仅支持前 72 字节的密码，超过会抛出异常
        try:
            if login_user.password and len(login_user.password.encode('utf-8')) > 72:
                logger.warning('密码长度不能超过72字节')
                raise LoginException(data='', message='密码长度不能超过72字节')
        except UnicodeError:
            # 极端情况下编码失败，按密码错误处理
            logger.warning('密码编码异常')
